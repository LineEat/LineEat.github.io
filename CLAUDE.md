...
## 使用者說「整理 inbox」時的步驟
（`organize_inbox.ps1` 每天 19:45 會以 `claude -p` 無人值守執行同一套步驟；無人值守時不要提問，
拿不準的欄位留空並在 notes 說明，最後的回報會被寫進 `organize.log`。）
...
2. 每則貼文抽出：營業日期、餐車名、類別（便當/小吃/飲料/甜點/其他）、品項與價格、地點、時段、
   ...
   - 相鄰截圖會重疊（每頁約半頁），同一則貼文只記一次（以餐車名 + 營業日期為鍵）。
   - `fulltext` 欄：把貼文全文逐字照抄，保留原本的換行（CSV 內用引號包住多行），表情符號可省略。
     截圖腳本會先點開「顯示更多」，所以全文通常是完整的；若仍看到「顯示更多」代表沒展開成功，
     抄到截斷處並在 notes 註明「內容截斷」，不要猜。
   - 置頂的公告貼文（二期小秘書、主辦等非餐車貼文）跳過。
3. 追加到 `data/trucks.csv`（UTF-8，欄位含逗號要加引號）。同一餐車同一營業日已存在就更新而不重複。
   `source_image` 填該貼文第一次出現的截圖歸檔後相對路徑。
3a. 圖片：對每次抓取跑 `python crop_images.py inbox <時間戳> tmp_crops`，它會裁出每則貼文的 DM 區塊
   （只含圖片，不含名稱與留言）到 `tmp_crops/`（已在 .gitignore）。逐一 Read 確認是哪台餐車，
   優先用沒標「被頁緣切到」的版本，存成 `docs/img/<營業日>_<餐車英文slug>.jpg`（JPEG 品質 85），
   並在 CSV 的 `images` 欄填 `img/檔名`（多張用「；」分隔）。公告類的白色卡片不是 DM，跳過。
   做完 `rm -r tmp_crops`。搬檔、刪檔要逐一寫完整路徑，不要用 `cd` 串接或萬用字元（無人值守白名單會擋）。
...
## 慣例
- **絕對不要刪除 `data/trucks.csv` 既有的列**，也不要刪 `docs/img` 既有圖片；貼文從 LINE 消失不代表資料要拿掉，這份表就是歷史紀錄。
